...
import { KanbanBoardContainer, KandbanBoard } from "@/components/tasks/kanban/board"
...
function List({ children }: React.PropsWithChildren) {
    ...
    const handleAddCard = (args: { stageId: string }) => {
        const path = args.stageId === 'unnassigned' ? '/tasks/new' : `/tasks/new?stageId=${args.stageId}`;
        replace(path)
    }
    ...
    return (
        <>
            <KanbanBoardContainer>
                <KandbanBoard onDragEnd={handleOnDragEnd}>
                    <KanbanColumn id="unnassigned"
                        title={"unassigned"}
                        count={taskStages.unnassignedStage.length || 0}
                        onAddClick={() => handleAddCard({ stageId: 'unnasigned' })}
                    >
                        {taskStages.unnassignedStage.map((task) => (<KanbanItem key={task.id} id={task.id} data={{ ...task, stageId: 'unnasigned' }}>
                            <ProjectCardMemo
                                {...task}
                                dueDate={task.dueDate || undefined}
                            />
                        </KanbanItem>))}
                        {!taskStages.unnassignedStage.length && (
